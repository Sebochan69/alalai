    ...
    <div className="p-5 md:p-8 max-w-5xl mx-auto w-full">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-7">
        <div>
          ...
          <h1 className="text-2xl font-black tracking-tight">Assigned Reports</h1>
          <p className="text-sm text-muted-foreground mt-0.5">All reports for Zones A, B, C</p>
        </div>
        <Link href="/admin/map">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-7">
        {miniStats.map((s) => (
          <div key={s.label} className={`bg-card border rounded-2xl p-3.5 shadow-sm flex items-center gap-3 ${s.border}`}>
            <div className={`w-8 h-8 rounded-xl ${s.bg} flex items-center justify-center shrink-0 ${s.color}`}>
              {s.icon}
            </div>
            <div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 flex-wrap mb-1">
                      <span className="text-[10px] font-mono text-muted-foreground bg-muted px-1.5 py-0.5 rounded">#{report.id}</span>
                      <span className="text-[10px] font-semibold text-muted-foreground bg-muted/60 border border-border px-2 py-0.5 rounded-full">{report.tagging}</span>
                      {report.priority && (
                    ...
                    <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-[11px] text-muted-foreground">{report.location}</span>
                      </div>
                      {report.citizenName && (
                        <div className="flex items-center gap-1">
                          <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-muted-foreground">
                          ...
                          <span className="text-[11px] text-muted-foreground">{report.citizenName}</span>
                        </div>
                      )}
                    </div>
                  </div>
