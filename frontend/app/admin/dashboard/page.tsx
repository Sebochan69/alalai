...
const STATUS: Record<string, { label: string; dot: string; badge: string }> = {
  pending:        { label: "Pending",      dot: "bg-amber-400",        badge: "bg-amber-500/12 text-amber-400 border-amber-500/25" },
  "under-review": { label: "Under Review", dot: "bg-blue-400",         badge: "bg-blue-500/12 text-blue-400 border-blue-500/25" },
  "in-progress":  { label: "In Progress",  dot: "bg-violet-400",       badge: "bg-violet-500/12 text-violet-400 border-violet-500/25" },
  resolved:       { label: "Resolved",     dot: "bg-emerald-400",      badge: "bg-emerald-500/12 text-emerald-400 border-emerald-500/25" },
  ...
  return (
    <div className="p-5 md:p-8 max-w-5xl mx-auto w-full">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-8">
        <div>
          <p className="text-[11px] font-bold uppercase tracking-widest text-violet-500 dark:text-violet-400 mb-1">Admin Dashboard</p>
          <h1 className="text-2xl font-black tracking-tight">Good day, Admin 1 👋</h1>
          <p className="text-sm text-muted-foreground mt-0.5">
            Managing <span className="text-violet-500 dark:text-violet-400 font-semibold">Zones A, B, C</span> — May 2026
          </p>
        </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <p className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground">Recent Reports</p>
            <span className="text-[10px] font-bold text-muted-foreground bg-muted px-2 py-0.5 rounded-full">{recent.length}</span>
          </div>
          <Link href="/admin/reports" className="text-xs text-violet-500 dark:text-violet-400 font-semibold hover:underline flex items-center gap-1">
            View all
        ...
        <div className="space-y-2">
          {recent.map((report) => {
            ...
            return (
              <Link key={report.id} href={`/admin/reports/${report.id}`} className="block group">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 flex-wrap mb-1">
                        ...
                        <span className="text-[10px] font-semibold text-muted-foreground bg-muted px-2 py-0.5 rounded-full border border-border">{report.tagging}</span>
                        {report.priority && (
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border capitalize ${PRIORITY_CHIP[report.priority] ?? ""}`}>{report.priority}</span>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-1 mt-1">
                        ...
                        <p className="text-[11px] text-muted-foreground truncate">{report.location}</p>
                      </div>
                    </div>
                    {/* Right: badge + date */}
                    <div className="flex flex-col items-end gap-1.5 shrink-0">
                      <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full border ${s.badge} flex items-center gap-1`}>
      ...
      {/* Monthly snapshot */}
      <div className="mt-8">
        <p className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground mb-4">Monthly Overview — April 2026</p>
        <div className="bg-card border border-border/60 rounded-2xl p-5 shadow-sm">
          <div className="grid grid-cols-3 divide-x divide-border/60 text-center">
            ...
            <div className="px-4 py-1">
              ...
              <p className="text-[11px] text-muted-foreground mt-1">Avg. Resolution</p>
            </div>
          </div>
          <div className="mt-4 pt-4 border-t border-border/40 flex items-center justify-between">
            ...
            <div className="flex-1 mx-4 bg-muted/40 rounded-full h-1.5 overflow-hidden">
              <div className="h-1.5 rounded-full bg-emerald-500" style={{ width: "75%" }} />
            </div>
            <Link href="/admin/monthly-report" className="text-xs text-violet-500 dark:text-violet-400 font-semibold hover:underline shrink-0">Full report →</Link>
          </div>
        </div>
      </div>
    </div>
