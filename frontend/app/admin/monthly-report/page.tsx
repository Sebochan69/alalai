...
export default async function MonthlyReportPage() {
  ...
  const resolved = Math.round(
    (report.overall_completion_rate / 100) * report.overall_complaint_count,
  );
  const unresolved = report.overall_complaint_count - resolved;

  const rateColor =
    report.overall_completion_rate >= 80
      ? "text-emerald-400"
      : report.overall_completion_rate >= 50
        ? "text-amber-400"
        : "text-rose-400";
      ...
      <div className="relative overflow-hidden border-b border-border/50">
        ...
        <div className="px-6 md:px-10 pt-8 pb-7 max-w-6xl mx-auto">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            ...
            >
              <div className={`w-2.5 h-2.5 rounded-full ${rateBarColor} animate-pulse`} />
      ...
      <div className="px-6 md:px-10 py-7 max-w-6xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="relative bg-card border border-violet-500/20 rounded-2xl p-5 shadow-sm overflow-hidden hover:-translate-y-0.5 hover:shadow-[0_8px_32px_rgba(139,92,246,0.2)] transition-all duration-200">
            ...
            <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-3">Total Filed</p>
            <p className="text-4xl font-black tracking-tight text-violet-400">{report.overall_complaint_count}</p>
            <p className="text-[10px] text-muted-foreground mt-1">complaints this month</p>
          </div>
          <div className="relative bg-card border border-emerald-500/20 rounded-2xl p-5 shadow-sm overflow-hidden hover:-translate-y-0.5 hover:shadow-[0_8px_32px_rgba(16,185,129,0.2)] transition-all duration-200">
            ...
            <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-3">Resolved</p>
            <p className="text-4xl font-black tracking-tight text-emerald-400">{resolved}</p>
            <p className="text-[10px] text-muted-foreground mt-1">fully closed</p>
          </div>
          <div className="relative bg-card border border-rose-500/20 rounded-2xl p-5 shadow-sm overflow-hidden hover:-translate-y-0.5 hover:shadow-[0_8px_32px_rgba(244,63,94,0.2)] transition-all duration-200">
            ...
            <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-3">Unresolved</p>
            <p className="text-4xl font-black tracking-tight text-rose-400">{unresolved}</p>
          ...
          <div className="relative bg-card border border-blue-500/20 rounded-2xl p-5 shadow-sm overflow-hidden hover:-translate-y-0.5 hover:shadow-[0_8px_32px_rgba(59,130,246,0.2)] transition-all duration-200">
            ...
            <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-3">Avg. Resolution</p>
            <p className="text-4xl font-black tracking-tight text-blue-400">
              {report.avg_solution_days}<span className="text-lg font-bold ml-0.5">d</span>
            </p>
            <p className="text-[10px] text-muted-foreground mt-1">per complaint</p>
          </div>
        </div>
        ...
        <div className="bg-card border border-border/60 rounded-2xl p-6 shadow-sm">
          <div className="flex items-end justify-between mb-4 gap-4 flex-wrap">
            <div>
              ...
              <div className="flex items-baseline gap-3">
                <span className={`text-5xl font-black tracking-tight ${rateColor}`}>
                  {report.overall_completion_rate}%
                </span>
            ...
            <div className="relative w-16 h-16 shrink-0">
              <svg viewBox="0 0 36 36" className="w-16 h-16 -rotate-90">
                <circle cx="18" cy="18" r="15.9" fill="none" stroke="currentColor" strokeWidth="3.5" className="text-muted/40" />
                <circle
                  cx="18" cy="18" r="15.9" fill="none" strokeWidth="3.5" strokeLinecap="round"
                  strokeDasharray={`${report.overall_completion_rate} ${100 - report.overall_completion_rate}`}
                  className={rateStrokeClass}
                />
              </svg>
              <span className={`absolute inset-0 flex items-center justify-center text-[10px] font-black ${rateColor}`}>
                {report.overall_completion_rate}%
              </span>
            </div>
          </div>
          ...
          <div className="h-4 bg-muted/30 rounded-full overflow-hidden flex">
            <div
              ...
              style={{ width: `${report.overall_completion_rate}%` }}
            />
            {unresolved > 0 && (
              <div
                className="h-4 bg-rose-500/30 transition-all duration-700 rounded-r-full"
                style={{ width: `${100 - report.overall_completion_rate}%` }}
              />
            )}
          </div>
          <div className="flex items-center gap-5 mt-3">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[11px] text-muted-foreground">Resolved ({resolved})</span>
            </div>
            <div className="flex items-center gap-1.5">
              <div className="w-2.5 h-2.5 rounded-full bg-rose-500/50" />
              <span className="text-[11px] text-muted-foreground">Unresolved ({unresolved})</span>
            </div>
          </div>
        </div>
            ...
            <div className="px-5 py-4 border-b border-border/50 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded-xl bg-violet-500/15 flex items-center justify-center shrink-0">
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-violet-500">
                    <line x1="18" y1="20" x2="18" y2="10" /><line x1="12" y1="20" x2="12" y2="4" /><line x1="6" y1="20" x2="6" y2="14" />
                  </svg>
                </div>
                <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground">By Category</p>
              </div>
              {topCategory && (
                ...
                return (
                  <div key={label}>
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <div className="flex items-center gap-2">
                        <span className={`text-[10px] font-black px-1.5 py-0.5 rounded-md border ${clr.light} ${clr.text}`}>
                          {count}
                        </span>
                        <span className="text-[10px] text-muted-foreground w-6 text-right tabular-nums">{pct}%</span>
                      </div>
                    </div>
                    <div className="h-1.5 bg-muted/40 rounded-full overflow-hidden">
                      <div className={`h-1.5 rounded-full ${clr.bar} transition-all duration-700`} style={{ width: `${Math.max(pct, 3)}%` }} />
                    </div>
                  </div>
                );
              })}
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="relative bg-card border border-violet-500/20 rounded-2xl p-5 shadow-sm overflow-hidden">
              ...
              <div className="relative">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <p className="text-[10px] font-bold uppercase tracking-widest text-violet-500 dark:text-violet-400">AI Forecast</p>
                ...
                <p className="text-sm text-muted-foreground leading-relaxed">{report.forecast}</p>
              </div>
            </div>
            ...
            <div className="bg-card border border-border/60 rounded-2xl overflow-hidden shadow-sm flex-1">
              <div className="px-5 py-4 border-b border-border/50 flex items-center gap-2">
                ...
                <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Suggested Actions</p>
              ...
              <div className="p-4 space-y-2.5">
                {report.suggest_actions.map((action, i) => (
                  <div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-muted/30 border border-border/40 hover:bg-muted/50 transition-colors">
