    ...
    <div className="space-y-4">
      ...
      <div className="bg-card border border-border/60 rounded-2xl overflow-hidden shadow-sm">
        <div className="px-5 py-3 border-b border-border/60 bg-muted/30">
          <p className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground">Update Status</p>
        </div>
        <div className="p-5 space-y-3">
          ...
          {status === "resolved" && (
            <div className="flex items-start gap-2 rounded-xl bg-emerald-500/8 border border-emerald-500/20 px-3 py-2.5">
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-emerald-400 mt-0.5 shrink-0">
                <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/>
              </svg>
              <p className="text-xs text-emerald-600 dark:text-emerald-400">Citizen will be notified by email to review and close the report.</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-card border border-border/60 rounded-2xl overflow-hidden shadow-sm">
        <div className="px-5 py-3 border-b border-border/60 bg-muted/30">
          <p className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground">
            Admin Comment <span className="normal-case font-normal">(optional)</span>
          </p>
        </div>
        <div className="p-5 space-y-3">
          {/* Previous comment */}
          {report.adminComment && (
            <div className="flex items-start gap-3 bg-muted/30 border border-border/50 rounded-xl p-3">
              <div className="w-7 h-7 rounded-full bg-violet-600/15 border border-violet-500/25 flex items-center justify-center text-violet-500 dark:text-violet-400 text-[10px] font-black shrink-0">A</div>
              <div>
                <p className="text-xs text-muted-foreground leading-relaxed">{report.adminComment}</p>
                {report.adminCommentDate && (
                  <p className="text-[10px] text-muted-foreground/60 mt-1">
                    {report.adminName ?? "Admin"} · {new Date(report.adminCommentDate).toLocaleDateString("en-PH", { month: "short", day: "numeric", year: "numeric" })}
                  </p>
                )}
              </div>
            </div>
          ...
          <textarea
            rows={3}
            placeholder={report.adminComment ? "Add a new comment (replaces previous)..." : "Write a message to the citizen..."}
